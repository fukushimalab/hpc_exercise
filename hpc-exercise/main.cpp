...
int main(const int argc, const char** argv)
{
	...
	//課題9
	//intの行列を２倍，1/2倍するときに，ビットシフトと除算や除数の逆数での乗算で計算し比較せよ．
	//なお，大きい行列サイズでないと，効果がでない場合がある．
	if (false)
	{
		const int loop = 1000;
	...
	//課題27
	//上の関数を用いて，データ構造の相互変換を確認せよ． ４ｘ４のフロート転置を手書きで考えてみよ． ８ｘ８の転置を考えてみよ．
	if(false)
	{
		const int size = 64;
#ifdef __GNUC__
	...
	//課題28
	//__m256i（int）型を_m256（flot）型に変換せよ．
	if(false)
	{
...
#elif _MSC_VER
		__declspec(align(32)) float a[size][size];
		__declspec(align(32)) float b[size][size];
		__declspec(align(32)) float c[size][size];
#endif
		//init
		for(int i = 0; i < size; i++)
		{
			for(int j = 0; j < size; j++)
			{
				a[i][j] = rand_32f(0, 100);
				b[i][j] = rand_32f(0, 100);
				c[i][j] = 0;
			}
		}
