...
### 演習2
カラー画像におけるボックスフィルタをスカラーで実装せよ．
このとき，入力画像のデータ型をfloatとunsigned char（uchar）の2種類とし，画像のデータ構造をAoSのままで処理を行う場合と，SoAに変換する場合の2種類の実装を行い，計算時間を測定せよ．
なお，データ構造変換の時間は含めなくてよい．
また，カラーループは展開してよい．
そして，各々の実装の計算時間を比較し，考察せよ．

実装すべきものは下記の4つになる．
- 入力画像float型・スカラ実装・AoS
- 入力画像float型・スカラ実装・SoA
- 入力画像uchar型・スカラ実装・AoS
...
### 演習3
演習2で実装したスカラーコードをOpenMPでスレッド並列化せよ．
また，並列化するループは画素ループとカーネルループとし，それぞれを比較せよ．
そして，対応する並列化なし実装とも比較を行い，並列化効率などの観点から，考察せよ．

実装すべきものは下記の4つである．
- 入力画像float型・スカラ実装・SoA・画素ループ並列化
- 入力画像float型・スカラ実装・SoA・カーネルループ並列化
- 入力画像uchar型・スカラ実装・SoA・画素ループ並列化
- 入力画像uchar型・スカラ実装・SoA・カーネルループ並列化
...
### 演習4
演習3で実装したコードをSIMD実装に拡張せよ．
実装は，前述のカーネルループ展開，画素ループ展開を実装せよ．
また，これらに対応するスカラ実装の計算時間を比較し，SIMD実装にしたことによる高速化率や，画素ループ展開とカーネルループ展開の効率の違いなどを考察せよ．
...
### 演習5
演習4で実装したfloat実装をuchar実装に拡張せよ．
そして，対応するfloat型の計算時間と比較をし，考察せよ．

実装に必要なものは，次の2つである．
- 入力画像uchar型・SIMD実装・SoA・画素ループ並列化・画素ループ展開
- 入力画像uchar型・SIMD実装・SoA・画素ループ並列化・カーネルループ展開
...
### 演習5
スカラー実装およびSIMD実装をセパラブルフィルタとして実装せよ．
そして，セパラブルフィルタによる高速化率が`O(r)`と`O(r^2)`との関係に従うか調べ，考察せよ．

必要な実装は次の4つである．
- 入力画像float型・スカラ実装・SoA・画素ループ並列化・画素ループ展開
- 入力画像float型・SIMD実装・SoA・画素ループ並列化・画素ループ展開
...
### 演習6
演習2から演習5までに次の高速化に寄与する要素の組み合わせを実装してきた．
