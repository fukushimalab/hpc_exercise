...
また，ucharからintへの変換は以下のようにもできる．
ただし`_mm256_cvtepu8_epi32`命令は引数に__m128iのSSEの値を取ることに注意が必要．
ucharからの型変換なら，ベクトル長32で変換可能だが，これくらいの軽い処理はメモリで律速しているため，以下のように書いても速度は概ね変わらない．
```cpp
uchar* s;
int* d;
for (int i = 0; i < size; i += 8)
