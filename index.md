...
　このムーアの法則は終焉を迎えつつあり，これからのアーキテクチャの変化は予測できない．
...
VisualStudioの使用に習熟した人のみの利用にとどめてください．
（そもそも動かない関数などを使用しているところが一部あります．VisualStudioで動かすためにはどうしたらいいのかの質問には答えません．）
...
また，発展と書いてある項目には課題はついていません．
より発展した学習がしたい人のための自習用となっている．
...
ax+bx+cx= (a+b+c)*x
```
これを一般化したものに[ホーナー法](https://ja.wikipedia.org/wiki/%E3%83%9B%E3%83%BC%E3%83%8A%E3%83%BC%E6%B3%95)があり，多項式での演算が効率化される．
...
#### 課題9
intの行列を整数で2倍，1/2倍するとき，浮動小数点で2.f倍，1/2.f倍するとき，ビットシフトと除算や除数の逆数での乗算で計算し計算時間を比較せよ．
...
![CPUとメモリの速度の変化](memorycpu.png "CPUとメモリの速度の変化")
...
なお，レジスタは，後ろから値が入っていくため，直観的に位置を指定するには，指定順序を逆にするsetrを用いる．
setもsetrも計算時間は変わらない．
...
また，0初期化用のレジスタを確保しておく(`__m256 zeros = _mm256_setzero_ps()`)よりも，`_mm256_setzero_ps()`で毎回0を入力したほうがだいたい速い．
